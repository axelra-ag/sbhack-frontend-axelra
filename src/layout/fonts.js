export const __LIGHT_FONT_SOURCE = require("../../assets/fonts/OpenSans-Light.ttf");
export const __REGULAR_FONT_SOURCE = require("../../assets/fonts/Proxima-Nova-Reg.ttf");
export const __SEMIBOLD_FONT_SOURCE = require("../../assets/fonts/OpenSans-SemiBold.ttf");
export const __BOLD_FONT_SOURCE = require("../../assets/fonts/Proxima-Nova-Bold.ttf");
export const __EXTRABOLD_FONT_SOURCE = require("../../assets/fonts/OpenSans-ExtraBold.ttf");

export const __FONTS = {
    LIGHT: "LIGHT",
    REGULAR: "REGULAR",
    SEMIBOLD: "SEMIBOLD",
    BOLD: "BOLD",
    EXTRABOLD: "EXTRABOLD"
};
